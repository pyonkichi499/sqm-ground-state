...
- `pytest-cov` で `sqm_ground_state` パッケージの coverage を計測する
- coverage は branch coverage を有効にする
- 最低ラインは 80% とする
- HTML レポートは `htmlcov/` に生成し、Git 管理外にする
...
- **前提**: OS に `Noto Sans CJK JP` がインストール済みであること（`pyproject.toml` 外の OS 依存）
