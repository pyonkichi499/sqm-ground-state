...
- ドメイン: `src/sqm_ground_state/potentials.py`（ポテンシャル V・V'・解析的に分かる量の単一の源）
- 数値カーネル: `src/sqm_ground_state/lattice.py`、`src/sqm_ground_state/langevin.py`、`src/sqm_ground_state/exact.py`、`src/sqm_ground_state/observables.py`、`src/sqm_ground_state/analysis.py`
- オーケストレーション: `src/sqm_ground_state/experiment.py`（解析解・有限差分・Parisi-Wu の 3 者比較を実行）と `src/sqm_ground_state/runner.py`（出力保存）
- 実行例: `examples/` 以下の各スクリプトは設定を渡して `run_experiment` を呼ぶだけ

`src/sqm_ground_state/action.py` と `src/sqm_ground_state/analytic.py` は従来の関数 API を保つ後方互換レイヤーで、
内部的には `src/sqm_ground_state/potentials.py` と `src/sqm_ground_state/lattice.py` に委譲する。
...
使用フォント名は `src/sqm_ground_state/experiment.py` の `PLOT_FONT_FAMILY` で定義している。
