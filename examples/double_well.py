...
である。量子スペクトルの閉じた解析解は一般にはないため、
解析的に分かる古典的な量を表示し、スペクトルは有限差分対角化と
Parisi-Wu 確率過程量子化で比較する。
...
from src.action import double_well_force, double_well_potential
from src.analytic import double_well_analytic_results
...
def main():
    ...
    # 1. シミュレーションパラメータ
    # ------------------------------------------------------------------
    lam = 1.0            # 井戸の深さ・障壁高さを決める係数
    v = 1.0              # 古典的な極小点は x = +/- v
    mass = 1.0           # 粒子の質量
    n_lattice = 100      # ユークリッド時間方向の格子点数
    a = 0.1              # 格子間隔（ユークリッド時間）
    epsilon = 0.003      # ランジュバンステップ幅（二重井戸では小さめにする）
    n_therm = 9000       # 熱化スイープ数
    n_configs = 5000     # 測定する配位数
    n_skip = 25          # 測定間に進めるランジュバンステップ数
    rng_seed = 321       # 再現性のための乱数シード
    max_tau = n_lattice // 2
    ...
    output_dir = make_output_dir("double_well", output_params)
    analytic = double_well_analytic_results(lam=lam, v=v, mass=mass)

    print("=" * 60)
    print("  確率過程量子化 -- 二重井戸ポテンシャル")
    print("=" * 60)
    print(f"  lambda    = {lam}")
    print(f"  v         = {v}")
    print(f"  mass      = {mass}")
    ...
    # ------------------------------------------------------------------
    # 2. 解析的に分かる量と有限差分対角化による比較用の数値解
    # ------------------------------------------------------------------
    print("\n解析的に分かる量:")
    print(f"  量子スペクトルの解析解: なし（{analytic['reason']}）")
    print(f"  古典的極小点:           x = {analytic['classical_minima']}")
    print(f"  障壁位置:               x = {analytic['barrier_position']}")
    print(f"  障壁高さ:               {analytic['barrier_height']:.6f}")
    print(f"  井戸底の局所振動数:     {analytic['local_harmonic_omega']:.6f}")
    print(f"  局所調和近似の E0:      {analytic['local_harmonic_E0_approx']:.6f}")
    ...
    # 大きすぎる tau では相関関数が小さくなり、統計ノイズの影響が増える。
    plateau_start = 2
    plateau_end = min(8, len(m_eff))
    ...
    summary_path = save_summary(
        ...
        output_params,
        {
            ...
            "classical_minima": analytic["classical_minima"],
            "barrier_height": analytic["barrier_height"],
            "local_harmonic_omega": analytic["local_harmonic_omega"],
            "local_harmonic_E0_approx": analytic["local_harmonic_E0_approx"],
            "x2_analytic": None,
    ...
    try:
        ...
        ax.set_title("二重井戸ポテンシャルの有効質量")
        ax.set_ylim(0, max(2.5 * exact_gap, 1.0))
        ax.legend()
        ...
        ax.set_title("二重井戸ポテンシャルの基底状態波動関数")
        ax.legend()
        ax.grid(True, alpha=0.3)
        fig.tight_layout()
